package com.zkkj.gps.gateway.ccs.mq.subscribe;
...
import com.zkkj.gps.gateway.ccs.dto.amqp.PushMsgDto;
...
public class ConsumerSubscribe {
    ...
    private Logger logger = LoggerFactory.getLogger(ConsumerSubscribe.class);
     ...
     * @throws Exception
     */
    public boolean consemerSubscribe(String amqpUserName,String amqpPassWord){
        try {
            logger.info("消息队列消费者：amqpUserName：【" + amqpUserName + "】；amqpPassWord：" + amqpPassWord + "】 \n");
            Connection connection = factory.getConnection("MQSubscriber",amqpUserName,amqpPassWord);
            ...
            channel.queueBind(declareOk.getQueue(), MqConstants.EXCHANGE_NAME, routingKey, new HashMap<>());
                ...
                public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                    try {
                        ...
                        if (!StringUtils.isEmpty(rcvMsgContent)){
                            ...
                            if (!ObjectUtils.isEmpty(rcvMonitorDto)){
                                //logger.info("消息推送对象：rcvMonitorDto【" + rcvMonitorDto.toString() + "】");
                                String url = "";
                                switch (rcvMonitorDto.getFlag()){
                                    case 1://事件
                                        url = MqConstants.consumerEventMsg;
                                        //sendMsgToThirdParty(rcvMonitorDto,MqConstants.consumerEventMsg);
                                        break;
                                    case 2://报警
                                        url = MqConstants.consumerAlarmMsg;
                                        //sendMsgToThirdParty(rcvMonitorDto,MqConstants.consumerAlarmMsg);
                                        break;
                                }
                                if (!StringUtils.isEmpty(url)){
                                    sendMsgToThirdParty(rcvMonitorDto,url,channel,envelope);
                                } else {
     ...
     * @param implName
     */
    private void sendMsgToThirdParty(RcvMonitorDto rcvMonitorDto,String implName,Channel channel,Envelope envelope){
        String appKey = rcvMonitorDto.getAppKey();
        ...
        if (!StringUtils.isEmpty(appKey)){
            if (MqConstants.appKeyUrlMap != null && MqConstants.appKeyUrlMap.size() > 0){
                ...
                logger.info("消息推送数据为空：MqConstants.appKeyUrlMap\n");
            }
        } else {
            logger.info("消息推送appKey为空：appKey\n");
        }
        if (ObjectUtils.isEmpty(rcvMonitorDto.getData()) || StringUtils.isEmpty(appKey) ||
                StringUtils.isEmpty(appSecret) || StringUtils.isEmpty(thirdPartyUrl)){
            logger.info("消息推送相关数据为空：【" + rcvMonitorDto.getData() + "；"+ appKey + "；" +appSecret + "；" + thirdPartyUrl + "】\n");
            return;
        }
        PushMsgDto pushMsgDto = new PushMsgDto();
        pushMsgDto.setAppKey(appKey);
        pushMsgDto.setAppSecret(appSecret);
        pushMsgDto.setData(rcvMonitorDto.getData());
