package com.zkkj.gps.gateway.watermeter.entity.business;

import com.zkkj.gps.gateway.watermeter.util.HexStringUtils;
import org.junit.Test;

public class P_43Test {
	private byte[] b = {
			(byte) 0xC0,
			0x31,
			0x34, 0x01, 0x13, 0x01, 0x00, 0x01,
			0x43,
			0x00,
			0x00, 0x0D,
			(byte) 0xC0, (byte) 0xA8, 0x03, 0x64, 0x58, 0x58,
			0x00, 0x00, 0x22, (byte) 0x90,
			(byte) 0xFF, 0x00,
			0x01,
			(byte) 0xFC,
			(byte) 0xD0
	};

	@Test
	public void encoderBody() throws Exception {
		P_43 p_43 = new P_43("340113010001", "192.168.3.100:8888", "00000729", 255, 1);
		System.out.println(HexStringUtils.toHexString(p_43.encoder()));
	}

	@Test
	public void decoderBody() throws Exception {
//		P_43 p_43 = new P_43(b);
//		System.out.println(p_43);
	}

}