...
public class Message<T extends AbstractBody> extends AbstractMessage<T> {
    ...
    public void setType(Integer type) {
        this.type = type;
        this.idType = ProtocolId.toEnum(type);
    }

    public ProtocolId getIdType() {
        return idType;
    }

    public void setIdType(ProtocolId idType) {
        this.idType = idType;
        this.type = idType.value;
    }

    @Property(index = 2, type = DataType.WORD, desc = "消息体属性")
    public Integer getBodyProperties() {
        if (bodyLength >= 1024) {
            log.error("The max value of msgLen is 1023, but :【" + bodyLength + "】");
        }
